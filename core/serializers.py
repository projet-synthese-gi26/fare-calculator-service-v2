...
class EstimateInputSerializer(serializers.Serializer):
    ...
    qualite_trajet = serializers.IntegerField(
        min_value=1, max_value=10,
        required=False,
        allow_null=True,
        help_text="Évaluation difficulté trajet (1-10)."
    )
    use_ml_only = serializers.BooleanField(
        default=False,
        required=False,
        help_text="Forcer l'utilisation du modèle ML uniquement."
    )
    ...
    def validate(self, attrs):
        ...
        # S'assurer que les nouveaux champs sont présents (même si None)
        attrs.setdefault('qualite_trajet', None)
        attrs.setdefault('use_ml_only', False)
...
class FeaturesUtiliseesSerializer(serializers.Serializer):
    ...
    nb_virages = serializers.IntegerField(allow_null=True, required=False)
    heure = serializers.CharField()
    meteo = serializers.IntegerField()
    type_zone = serializers.IntegerField()
    qualite_trajet = serializers.IntegerField(allow_null=True, required=False)
